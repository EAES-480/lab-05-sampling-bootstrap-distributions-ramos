...
> *le has the largest SD, this implies that the processes that drives le have higher variability compared to fc and gpp*
...
> *Variables with a high population SD relative to their mean are mathematically harder to estimate. In this case viability is given by seasonality, because random chance might select extreme values, moving the sample mean.*
...
> *Relative error decreases as n increases. The improvement from 25 to 100 is much larger than from 300 to 1000 because the median relative error drops dramatically at lower sample sizes. Accuracy shows diminishing returns because the SE shrinks proportionally to the square root of the sample size.*
...
> *Increasing n will narrow the distribution and decrease the relative error. Increasing reps creates a more complete approximate sampling distribution*
...
> *The bootstrap distribution and the sampling distribution share a similar bell shape, but the bootstrap distribution is centered around the sample mean instead of the population mean. The bootstrap is trying to approximate the uncertainty. This is especially useful in EaES because we almost never observe an entire population and rarely have the resources to repeat a full field campaign to gather multiple distinct samples.*
...
> *Yes, the bootstrap SD represents the standard error of the sample mean, estimating how much we expect our sample mean to fluctuate due to random sampling.*
...
> *Yes, the bootstrap distribution looks very narrow and "confident," despite being centered. Precision is represented by the narrow spread of the bootstrap distribution, while accuracy is how close the estimate is to the true population value. Summer-only sampling introduces systematic bias.*
...
```{r ex3_stratified_sample, echo=TRUE, eval=T}
set.seed(480)
...
```{r ex3_strat_bootstrap, echo=TRUE, eval=T}
set.seed(480)
...
```{r ex3_plot_strat_boot, echo=TRUE, eval=T}
...
> *The random and stratified bootstrap distributions are centered accurately near the true population mean, whereas the summer-only distribution is centered far off. The stratified sampling design is the most accurate for estimating the annual mean because it guarantees observations across all seasons, overcoming strong seasonality. This teaches us that the bootstrap measures uncertainty around your estimate given the specific sample you observed, but it cannot correct a biased study design.*
...
```{r ex1_boot_ci_mean, echo=TRUE, eval=T}
...
*Yes, this tells us that the bootstrap method successfully quantified the uncertainty the sample estimate.*
...
```{r ex2_plot_ci, echo=TRUE, eval=T}
...
tibble(boot_mean = boot_means_random) %>%
  ...
  labs(
    x = "Bootstrap mean (swc)",
    y = "Count",
    subtitle = "Red lines = 2.5% - 97.5% confidence interval"
  )
```
...
> *They represent the lower and upper bounds of the 95% bootstrap confidence interval.*
...
```{r ex3_fit_model, echo=TRUE, eval=T}
...
```{r ex3_bootstrap_lm, echo=TRUE, eval=T}
set.seed(480)
...
```{r ex3_ci_params, echo=TRUE, eval=T}
...
> *It doesn't include zero. This implies that there is a statistically significant, negative linear relationship between the variables.*
